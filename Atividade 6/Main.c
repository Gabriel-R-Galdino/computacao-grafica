...
TipoDanca tipoDancaAtual = DANCA_PADRAO;
...
// Animação da dancinha
int ativarDancinha = 0;
...
void Dancinha(void);
void DesenhaChapeuJunino(void);
void DesenhaFogueira(void);
void ResetarPosicao(void);
...
void DesenhaChapeuJunino() {
    glPushMatrix();
    glTranslatef(0.0f, 0.4f, 0.0f);  // Posiciona no topo da cabeça
    glRotatef(anguloCabeca, 0, 0, 1); // Segue o movimento da cabeça

    // Aba do chapéu (parte circular)
    glColor3f(0.82f, 0.70f, 0.35f); // Cor palha
    glBegin(GL_TRIANGLE_FAN);
        glVertex2f(0.0f, -0.1f); // Centro
        for(int i = 0; i <= 360; i += 20) {
            float rad = i * M_PI / 180.0f;
            glVertex2f(cos(rad) * 0.35f, sin(rad) * 0.1f - 0.15f);
        }
    glEnd();

    // Corpo do chapéu (cilindro)
    glColor3f(0.9f, 0.8f, 0.5f); // Cor palha mais clara
    glBegin(GL_QUAD_STRIP);
        for(int i = 0; i <= 360; i += 30) {
            float rad = i * M_PI / 180.0f;
            glVertex2f(cos(rad) * 0.25f, sin(rad) * 0.05f + 0.05f);
            glVertex2f(cos(rad) * 0.25f, sin(rad) * 0.05f - 0.1f);
        }
    glEnd();

    // Topo do chapéu
    glBegin(GL_TRIANGLE_FAN);
        glVertex2f(0.0f, 0.15f); // Topo
        for(int i = 0; i <= 360; i += 20) {
            float rad = i * M_PI / 180.0f;
            glVertex2f(cos(rad) * 0.25f, sin(rad) * 0.05f + 0.05f);
        }
    ...
    // Faixa vermelha - POSIÇÃO REBAIXADA
    float posicaoYFaixa = -0.08f; 
    float alturaFaixa = 0.06f;    // Altura da faixa
    
    glColor3f(0.8f, 0.1f, 0.1f); // Vermelho
    
    // Faixa preenchida
    glBegin(GL_QUAD_STRIP);
        for(int i = 0; i <= 360; i += 30) {
            float rad = i * M_PI / 180.0f;
            float raioExterno = 0.28f;
            float raioInterno = 0.25f;
            glVertex2f(cos(rad) * raioExterno, sin(rad) * 0.05f + posicaoYFaixa);
            glVertex2f(cos(rad) * raioInterno, sin(rad) * 0.05f + posicaoYFaixa + alturaFaixa);
        }
    glEnd();
    
    // Contorno superior
    glColor3f(0.6f, 0.0f, 0.0f); // Vermelho escuro
    glLineWidth(3.0f);
    glBegin(GL_LINE_LOOP);
        for(int i = 0; i <= 360; i += 30) {
            float rad = i * M_PI / 180.0f;
            glVertex2f(cos(rad) * 0.28f, sin(rad) * 0.05f + posicaoYFaixa);
        }
    glEnd();
    ...
    glBegin(GL_LINE_LOOP);
        for(int i = 0; i <= 360; i += 30) {
            float rad = i * M_PI / 180.0f;
            glVertex2f(cos(rad) * 0.25f, sin(rad) * 0.05f + posicaoYFaixa + alturaFaixa);
        }
    glEnd();
    glLineWidth(1.0f);
...
void DesenhaCabeca() {
    ...
    glDisable(GL_LINE_SMOOTH);
    glLineWidth(1.0f);
    glScalef(0.7f, 0.7f, 1.0f); // Redimensiona para o tamanho original
    DesenhaChapeuJunino();
    glPopMatrix();
}
...
void DesenhaFogueira() {
    static float frame = 0.0f;
    frame += 0.05f; // Controla a animação
    
    glPushMatrix();
    glTranslatef(0.7f, -0.3f, 0.0f); // Posição da fogueira
    
    // Troncos
    glScalef(1.5f, 1.5f, 1.0f); // Aumenta o tamanho dos troncos
    ...
    glBegin(GL_TRIANGLE_FAN);
        ...
        glVertex2f(0.0f, 0.2f + sin(frame)*0.03f); // Centro animado
        
        glColor3f(1.0f, 0.5f, 0.0f); // Laranja
        for(int i = 0; i <= 360; i += 30) {
            float rad = i * M_PI / 180.0f;
            float raio = 0.1f + sin(frame + i*0.1f)*0.03f; // Chamas ondulantes
            glVertex2f(cos(rad) * raio, sin(rad) * raio * 1.5f);
        }
    glEnd();
...
void DesenhaCenario() {
   ...
   DesenhaFogueira(); // Desenha a fogueira
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'v': // ombro esquerdo rotaciona -90 graus
            ...
            rotOmbroEsq = clamp(rotOmbroEsq, OMBRO_MIN, OMBRO_MAX);
            break;
        case 'p': // Ativa/desativa rebolado padrão
            if (ativarDancinha) {
            if (tipoDancaAtual != DANCA_PADRAO) {
            // Se estava em outro estilo, reseta antes de mudar
                ResetarPosicao();
                tipoDancaAtual = DANCA_PADRAO;
            } else {
            // Se já estava no estilo padrão, desativa
                ativarDancinha = 0;
                ResetarPosicao(); // Reset ao desligar
            }
            } else {
                // Ativa o modo padrão
                ativarDancinha = 1;
        ...
        case 'o': // Ativa/desativa rebolado estilo 2
            if (ativarDancinha) {
            if (tipoDancaAtual != DANCA_ESTILO2) {
                // Se estava em outro estilo, reseta antes de mudar
                ResetarPosicao();
                tipoDancaAtual = DANCA_ESTILO2;
            } else {
                // Se já estava no estilo 2, desativa
                ativarDancinha = 0;
                ResetarPosicao(); // Reset ao desligar
            }
            } else {
                // Ativa o modo estilo 2
                ativarDancinha = 1;
                tipoDancaAtual = DANCA_ESTILO2;
            }
            break;
...
void Dancinha() {
    ...
    static float ombroDirOffset = 0.0f;
    static int ombroDirDirecao = 1;
    const float passoDistancia = 0.05f;
    const int maxPassos = 2;

    if (ativarDancinha) {
    
        // Barriga
        animacaoRebolado += direcaoRebolado * 0.004f;
        if (animacaoRebolado > 0.04f || animacaoRebolado < -0.04f) {
            direcaoRebolado *= -1;
        }
        
        // Pernas
        animacaoPerna += direcaoPerna * 0.005f;
        if (animacaoPerna > 0.03f || animacaoPerna < -0.03f) {
            direcaoPerna *= -1;
        }

        // Asas
        animacaoAsa += direcaoAsa * 0.0005f;
        if (animacaoAsa > 0.03f || animacaoAsa < -0.03f) {
            direcaoAsa *= -1;
        }

        // Cauda
        animacaoCauda += direcaoCauda * 0.0005f;
        if (animacaoCauda > 0.03f || animacaoCauda < -0.03f) {
            direcaoCauda *= -1;
        }

        // Movimento lateral
        if (fabs(translateX - targetX) < 0.001f) {
            passos++;
            if (passos >= maxPassos * 2) passos = 0;
            targetX += (passos < maxPassos) ? -passoDistancia : passoDistancia;
        }

        translateX += (targetX - translateX) * 0.1f;
        translateX = clamp(translateX, SCENE_LEFT + (POKEMON_WIDTH * scaleFactor)/2, 
                                  SCENE_RIGHT - (POKEMON_WIDTH * scaleFactor)/2);

        // Animação São joão
        if (tipoDancaAtual == DANCA_ESTILO2) {
            ombroDirOffset += 0.8f * ombroDirDirecao;
            if (ombroDirOffset > 15.0f || ombroDirOffset < -15.0f) {
                ombroDirDirecao *= -1;
            }
        }

        // Configuração dos braços baseada no parangolé
        float speed = 0.05f;
        float targetOmbroDir, targetCotoveloDir, targetMaoDir;
        float targetOmbroEsq, targetCotoveloEsq, targetMaoEsq;

        if (tipoDancaAtual == DANCA_PADRAO) {
            // Posição Parangolé
            targetOmbroDir = targetOmbroEsq = 60.0f;
            targetCotoveloDir = targetCotoveloEsq = 130.0f;
            targetMaoDir = targetMaoEsq = 180.0f;
        } else {
            // Posição São João
            targetOmbroDir = -60.0f + ombroDirOffset;
            targetCotoveloDir = 10.0f;
            targetMaoDir = -40.0f;
            
            targetOmbroEsq = -20.0f + ombroDirOffset;
            targetCotoveloEsq = 0.0f;
            targetMaoEsq = 110.0f;
        }

        // Aplica aos braços direitos
        rotOmbroDir += (targetOmbroDir - rotOmbroDir) * speed;
        rotCotoveloDir += (targetCotoveloDir - rotCotoveloDir) * speed;
        rotMaoDir += (targetMaoDir - rotMaoDir) * speed;

        // Aplica aos braços esquerdos
        rotOmbroEsq += (targetOmbroEsq - rotOmbroEsq) * speed;
        rotCotoveloEsq += (targetCotoveloEsq - rotCotoveloEsq) * speed;
        rotMaoEsq += (targetMaoEsq - rotMaoEsq) * speed;
